...
Encryption::Encryption() : loop_(10), file_size_(0), buffer_size_(0)
{
	file_path_ = ".";
...
void Encryption::xor_block(uint32_t & source, const size_t random)
{
    source = source ^ random;
}

void Encryption::xor_block(CryptCell32 & source, const size_t random)
{
...
void Encryption::xor_block(uint64_t & source, const uint64_t random)
{
    source = source ^ (random * 18446744073709551);
}

void Encryption::xor_block(CryptCell64 & source, const uint64_t random)
{
...
{
	std::for_each(buffer_.begin() + ((buffer_size_ / cpu_) * i), buffer_.end() - ((buffer_size_ / cpu_) * (cpu_ - 1 - i)),
                  [=](MimicIntCryptCell64 &n) { xor_block(n.num, random_num); });
}

void Encryption::set_file_path(std::string path)
{
    file_path_ = path;
}
...
void Encryption::set_key()
{
	if (key_.empty())
	{
		...
		for (size_t i = 0; i < loop_; i++)
		{
            ...
            key_ += rand() % 5 != 0 ? 'S' + std::to_string(random_num) : 'X' + std::to_string(random_num);
		}
	}
}
...
std::string Encryption::get_file_path() const
{
    return std::string(file_path_);
}
...
bool Encryption::get_ready(bool decrypt)
{
	...
	else 
	{
        ...
        if (!fout_.is_open())
        {
            fout_.open(file_path_ +=".ldecrypt", std::ios::binary);
        }
	}
	return true;
}
