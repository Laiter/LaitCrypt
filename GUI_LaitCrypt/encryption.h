...
public:
    ...
    void xor_block(uint32_t& source, const size_t random);
    void xor_block(CryptCell32& source, const size_t random);
#else
	void switch_shift(CryptCell64& source, const size_t random, const bool decrypt = 0);
    void xor_block(uint64_t& source, const uint64_t random);
    void xor_block(CryptCell64& source, const uint64_t random);
#endif
